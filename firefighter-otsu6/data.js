...
const StudyOS = (() => {
  ...
  let keyPointsCache = null;
  let trueFalseCache = null;

  async function loadQuestions() {
    if (cache) return cache;
    if (window.STUDY_OS_DATA) {
      cache = _excludeUnsellable(window.STUDY_OS_DATA.questions);
      return cache;
    }
  ...
  async function loadIntroQuestions() {
    ...
    if (window.STUDY_OS_DATA) return (introCache = window.STUDY_OS_DATA.introQuestions);
  ...
  async function loadReleases() {
    ...
    if (window.STUDY_OS_DATA) return (releasesCache = window.STUDY_OS_DATA.releases);
  ...
  async function loadKeyPoints() {
    if (keyPointsCache) return keyPointsCache;
    if (window.STUDY_OS_DATA) return (keyPointsCache = window.STUDY_OS_DATA.keyPoints);
    const res = await fetch("data/key_points.json", { cache: "no-store" });
    if (!res.ok) throw new Error(`重要ポイントを読み込めませんでした (${res.status})`);
    keyPointsCache = await res.json();
    return keyPointsCache;
  }

  async function loadTrueFalse() {
    if (trueFalseCache) return trueFalseCache;
    if (window.STUDY_OS_DATA) return (trueFalseCache = window.STUDY_OS_DATA.trueFalse);
    const res = await fetch("data/true_false.json", { cache: "no-store" });
    if (!res.ok) throw new Error(`○×問題を読み込めませんでした (${res.status})`);
    trueFalseCache = await res.json();
  ...
  function getStats(questions) {
    ...
    const relevantAnswers = Object.entries(progress.answers).filter(([id]) => questionIds.has(id));
    const answered = relevantAnswers.length;
    let totalAttempts = 0, totalCorrect = 0;
    relevantAnswers.forEach(([, h]) => {
  ...
  return {
    loadQuestions, loadIntroQuestions, loadReleases, loadKeyPoints, loadTrueFalse, invalidateCache,
