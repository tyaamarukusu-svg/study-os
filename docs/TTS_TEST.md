...
女性寄りの `marin` と男性寄りの `cedar` を、同じ1問で聞き比べます。対応モデルでは、明るく前向きな講師風の読み方を指示します。

```sh
python3 scripts/generate_voice_comparison.py
```

生成後は `voice-comparison.html` を開きます。問題の後には、ブラウザ側で4回の小さなカウント音を鳴らしてから正解・解説を再生します。カウント音にAPI料金はかかりません。
